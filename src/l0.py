...
class HardConcreteGate(nn.Module):
    ...
    def get_continuous_mask(self) -> torch.Tensor:
        """Returns the continuous soft gate values (0 to 1) without hard thresholding."""
        with torch.no_grad():
            s = torch.sigmoid(self.log_alpha)
            s_stretched = s * (self.zeta - self.gamma) + self.gamma
            return F.hardtanh(s_stretched, min_val=0, max_val=1)
