...
def load_data(filepath, test_size=0.2, val_size=0.1, random_state=42):
    ...
    train_val, test = train_test_split(df, test_size=test_size, stratify=df['cancer_type'], random_state=random_state)
    train, val = train_test_split(train_val, test_size=val_size / (1 - test_size), stratify=train_val['cancer_type'], random_state=random_state)
