...
class LLMIsDefault:
    ...
    def _forward_func(self, inputs_embeds: torch.Tensor, attention_mask: torch.Tensor = None, target_token_index: int = -1):
        """
        Custom forward function for Captum.
        """
        if attention_mask is not None and attention_mask.shape[0] != inputs_embeds.shape[0]:
    ...
    def interpret(self, 
                  ...
                  n_steps: int = 50,
                  internal_batch_size: int = None) -> Dict[str, Any]:
