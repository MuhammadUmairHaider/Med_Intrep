...
class ProbingClassifier:
    ...
    def train(self, activations: np.ndarray, labels: np.ndarray):
        X_train, X_test, y_train, y_val = train_test_split(activations, labels, test_size=0.2, random_state=42)
    ...
    def save(self, save_dir: str):
        ...
        path = os.path.join(save_dir, f"probe_layer_{self.layer_idx}_{self.concept_name}.joblib")
        joblib.dump(self.model, path)

def run_layer_wise_probing(activations_per_layer: Dict[int, np.ndarray], labels: np.ndarray, concept_name: str) -> Dict[int, float]:
    """
