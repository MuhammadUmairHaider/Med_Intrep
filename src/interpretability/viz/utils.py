...
def plot_token_importance(token_list: List[str], importance_scores: List[float], title: str = "Token Importance", save_path: Optional[str] = None):
    """
    ...
    sns.barplot(x='Score', y='Token', data=df, palette=colors, hue='Token', legend=False)
    plt.title(title)
...
def plot_text_heatmap(tokens: List[str], scores: List[float], title: str = "Text Heatmap", save_path: Optional[str] = None):
    """
    ...
    html_content = f"<h3>{title}</h3><div style='font-family: monospace; line-height: 1.5;'>"
    ...
    for token, score in zip(tokens, norm_scores):
        ...
        html_content += f"<span style='background-color: {color}; padding: 0 2px; margin: 0 1px; border-radius: 3px;'>{display_token}</span>"
...
def plot_latent_space(embeddings: np.ndarray, labels: List[str], method: str = "t-SNE", save_path: Optional[str] = None):
    """
    ...
    sns.scatterplot(x='x', y='y', hue='Label', data=df, palette='viridis', s=100, alpha=0.8, edgecolor='k')
...
def plot_probing_accuracy(layers: List[int], accuracies: List[float], title: str = "Probing Accuracy per Layer", save_path: Optional[str] = None):
    """
